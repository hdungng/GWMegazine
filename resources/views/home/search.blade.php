    ...
    <div class="col-md-9 col-lg-7">
        ...
        <div id="content" class="ms-xl-0 ms-lg-5 p-4 p-md-5 pt-5">
            <div class="ms-4 d-flex justify-content-between flex-column flex-sm-row">
                <h2 class="fw-semibold fs-5">SEARCH KEYWORD <span class="ms-3 keyword">"{{ $query }}"</span>
                </h2>
                                        ...
                                        <div class="col-8">
                                            <div class="card-body">
                                                <h3 class="card-title fw-semibold">
                                                    @if (str_contains($contribution->title, $query))
                                                        {!! str_replace($query, "<span class='highlight'>$query</span>", $contribution->title) !!}
                                                    @else
                                                        {{ $contribution->title }}
                                                    @endif
                                                </h3>
                                                <p class="card-desc">{{ $contribution->description }}</p>
                                                <p class="card-author"><span>
                                                        @if (str_contains($contribution->student_name, $query))
                                                            {!! str_replace($query, "<span class='highlight'>$query</span>", $contribution->student_name) !!}
                                                        @else
